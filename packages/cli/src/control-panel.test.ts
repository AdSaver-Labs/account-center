...
test("local control panel serves an accessible application shell", async () => {
  ...
  try {
    ...
    assert.match(html, /id="audit-from" name="from" type="date"/);
    assert.match(html, /<label for="audit-to">Recorded through \(UTC\)<\/label>/);
    assert.match(html, /id="audit-to" name="to" type="date"/);
    ...
    assert.match(html, /<label for="operation-from">Recorded from \(UTC\)<\/label>/);
    assert.match(html, /id="operation-from" name="from" type="date"/);
    assert.match(html, /<label for="operation-to">Recorded through \(UTC\)<\/label>/);
    assert.match(html, /id="operation-to" name="to" type="date"/);
    ...
    assert.match(html, /parameters\.set\('from', operationFrom\.value\)/);
    assert.match(html, /parameters\.set\('to', operationTo\.value\)/);
    ...
    assert.match(html, /function isLimitsInventory\(data\)/);
    assert.match(html, /function isModelCatalog\(data\)/);
    assert.match(html, /function isModelSelectionEvidence\(selection\)/);
    assert.match(html, /key === 'models'\) valid = valid && isModelCatalog\(result\.value\)/);
    assert.match(html, /key === 'limits'\) valid = valid && isLimitsInventory\(result\.value\)/);
    assert.match(html, /Requested policy: Not reported/);
    assert.match(html, /Effective runtime model: Not reported/);
    assert.match(html, /Fallback chain: Not reported/);
    ...
    assert.match(html, /function renderAccountsRouting\(limitData, capabilityData, unavailable\)/);
    assert.match(html, /Selected-scope limit inventory could not be verified/);
    assert.match(html, /No connected accounts were reported by the protected selected-scope inventory\./);
    assert.match(html, /renderAccountsRouting\(data\.limits, data\.capabilities, unavailable\)/);
