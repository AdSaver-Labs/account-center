...
import { AccountCenterStatus, AuditRecord, AuditStore, AuthChallengeStore, createRuntimeAdapter, executeAccountCenterCommand, MutationRepository, publicStatusView } from "@account-center/core";
...
export function createAccountCenterServer(options: AccountCenterServerOptions) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (pathname === "/api/models") {
      ...
      if (!query) return send(response, 400, { error: "invalid_query" });
      const adapter = createRuntimeAdapter(source);
