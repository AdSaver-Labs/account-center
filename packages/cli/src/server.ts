  ...
  <script>
    (function () {
      ...
      function renderWorkspace(data, unavailable) {
        ...
        var operations = data.operations && data.operations.operations || []; operationRecords.innerHTML = unavailable.operations ? unavailableRecord('Operation history') : operations.length ? operations.map(operationRecord).join('') : '<p class="empty">No completed protected operations are available.</p>';
      }
