...
async function auditRecordDetail(store: AuditStore | undefined, id: string): Promise<unknown | undefined> {
  const record = store ? (await store.list({ limit: 1_000 })).find((candidate) => candidate.id === id) : undefined;
  return record ? {
...
<body>
  ...
  <script>
    (function () {
      ...
      async function loadOperationDetail(id) { return api('/api/mutation-operations/' + encodeURIComponent(id)); }
      ...
      async function loadAuditDetail(id) { return api('/api/audit/' + encodeURIComponent(id)); }
