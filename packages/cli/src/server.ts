...
<body>
  ...
  <script>
    (function () {
      ...
      function modelSelectionEvidence(selection) { var absent = { requestedPolicy: { state: 'not_reported' }, effectiveRuntimeModel: { state: 'not_reported' }, fallbackChain: { state: 'not_reported' }, verificationState: 'UNPROVEN' }; if (!selection || typeof selection !== 'object' || !selection.requestedPolicy || !selection.effectiveRuntimeModel || !selection.fallbackChain || selection.requestedPolicy.state !== 'not_reported' || selection.effectiveRuntimeModel.state !== 'not_reported' || selection.fallbackChain.state !== 'not_reported') return absent; return { requestedPolicy: selection.requestedPolicy, effectiveRuntimeModel: selection.effectiveRuntimeModel, fallbackChain: selection.fallbackChain, verificationState: selection.verificationState === 'UNPROVEN' ? 'UNPROVEN' : 'UNPROVEN' }; }
