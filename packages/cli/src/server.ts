...
export function createAccountCenterServer(options: AccountCenterServerOptions) {
  ...
  const issuedHandoffs = new Set<string>();
  ...
  const server = createServer({ maxHeaderSize: 12_288 }, async (request, response) => {
    ...
    if (request.method === "POST" && new URL(request.url ?? "/", "http://account-center.local").pathname === "/api/auth-handoffs/openclaw/preflight") {
      ...
      if (!isObservedRuntimeScope(status, { runtime: "openclaw", scope: "default" })) return send(response, 400, { error: "unknown_runtime_scope" });
      const view = nativeAuthHandoffPreflightView(body); issuedHandoffs.add(view.handoffId);
    ...
    if (nativeHandoffId) {
      ...
      if (!isEmptyRecord(body) || !issuedHandoffs.has(nativeHandoffId)) return send(response, 400, { error: "invalid_native_auth_handoff_recheck" });
      const status = await serverStatus();
      if (!status) return send(response, 503, { error: "status_unavailable" });
...
function nativeAuthHandoffRecheckId(path: string | undefined): string | undefined { return path?.match(/^\/api\/auth-handoffs\/openclaw\/(handoff_[a-f0-9]{32})\/recheck$/)?.[1]; }
...
<body>
  ...
  <script>
    (function () {
      ...
      function renderSentinelRuntimes(connectionData, unavailable) { var profiles = latestStatus && Array.isArray(latestStatus.profiles) ? latestStatus.profiles : []; var routeList = latestStatus && Array.isArray(latestStatus.routes) ? latestStatus.routes : []; var generatedAt = latestStatus && isTimestamp(latestStatus.generatedAt) ? latestStatus.generatedAt : 'Not reported'; sentinelRuntimes.innerHTML = ['hermes', 'openclaw', 'codex'].map(function (runtime) { var state = unavailable.connections && runtime !== 'codex' ? 'unproven' : sentinelRuntimeState(runtime, connectionData); var route = routeList.filter(function (item) { return item.runtime === runtime; })[0]; var active = route && typeof route.activeProfileId === 'string' ? route.activeProfileId : 'Not reported'; var profile = route ? profiles.filter(function (item) { return item.id === active; })[0] : undefined; var compatible = profiles.filter(function (item) { return Array.isArray(item.runtimeCompatibility) && item.runtimeCompatibility.indexOf(runtime) !== -1; }); var weekly = profile && profile.usage && Array.isArray(profile.usage.windows) ? profile.usage.windows.filter(function (window) { return window.name === 'weekly'; })[0] : undefined; var availability = weekly && typeof weekly.remainingPct === 'number' ? weekly.remainingPct + '% remaining' : state === 'unproven' ? 'UNPROVEN — not reported' : compatible.length ? 'UNPROVEN — no active route capacity' : 'Unavailable'; var verified = state === 'connected' || state === 'available' ? generatedAt : 'UNPROVEN — not verified'; var label = state === 'needs-auth' ? 'Needs auth' : state === 'connected' ? 'Connected' : state === 'available' ? 'Available' : state === 'unavailable' ? 'Unavailable' : 'UNPROVEN'; return '<article class="account-record" role="listitem"><div class="account-name">' + escapeHtml(runtime) + '</div><dl class="account-details"><div><dt>State</dt><dd>' + escapeHtml(label) + '</dd></div><div><dt>Active route</dt><dd>' + escapeHtml(active) + '</dd></div><div><dt>Weekly availability</dt><dd>' + escapeHtml(availability) + '</dd></div><div><dt>Last verified</dt><dd>' + escapeHtml(verified) + '</dd></div></dl></article>'; }).join(''); }
      function renderWorkspace(data, unavailable) {
        ...
        renderSentinelRuntimes(data.connections, unavailable);
      ...
      nativePreflight.addEventListener('click', async function () { if (selectedRuntime() !== 'openclaw' || !token.value) return; nativePreflight.disabled = true; nativeStatus.textContent = 'Preflight running; result remains UNPROVEN.'; try { var result = await api('/api/auth-handoffs/openclaw/preflight', { method: 'POST', body: JSON.stringify({ runtime: 'openclaw', provider: 'openai', scope: 'default', idempotencyKey: 'panel_' + Date.now().toString(36) + Math.random().toString(36).slice(2) }) }); nativeHandoffId = result.handoffId || ''; nativeRecheck.disabled = !nativeHandoffId; nativeStatus.textContent = 'Native sign-in required — UNPROVEN. Complete native sign-in outside Account Center, then recheck.'; } catch (_) { nativeStatus.textContent = 'UNPROVEN — preflight could not be verified.'; } finally { nativePreflight.disabled = selectedRuntime() !== 'openclaw'; } });
      nativeRecheck.addEventListener('click', async function () { if (!nativeHandoffId) return; nativeRecheck.disabled = true; nativeStatus.textContent = 'Rechecking read-only status…'; try { var result = await api('/api/auth-handoffs/openclaw/' + encodeURIComponent(nativeHandoffId) + '/recheck', { method: 'POST', body: '{}' }); nativeStatus.textContent = result.state === 'postflight_unproven' ? 'UNPROVEN — no sign-in change was verified.' : 'UNPROVEN — recheck response was not actionable.'; } catch (_) { nativeStatus.textContent = 'UNPROVEN — recheck could not be verified.'; } });
      openOnboarding();
    }());
  </script>
</body>
