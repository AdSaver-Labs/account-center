...
<body>
  ...
  <main class="shell" id="workspace" tabindex="-1">
    ...
    <nav class="tabs" aria-label="Account Center views" role="tablist"><button class="tab" type="button" role="tab" aria-selected="true" aria-controls="dashboard-view" id="dashboard-tab" data-tab="dashboard">Dashboard</button><button class="tab" type="button" role="tab" aria-selected="false" aria-controls="guided-view" id="guided-tab" data-tab="guided">Guided auth</button><button class="tab" type="button" role="tab" aria-selected="false" aria-controls="catalogs-view" id="catalogs-tab" data-tab="catalogs">Runtime catalogs</button><button class="tab" type="button" role="tab" aria-selected="false" aria-controls="audit-view" id="audit-tab" data-tab="audit">Receipts &amp; audit</button></nav>
  ...
  <script>
    (function () {
      ...
      var guidedRecords = document.getElementById('guided-records'); var scopeRecords = document.getElementById('scope-records'); var catalogModels = document.getElementById('catalog-models'); var auditRecords = document.getElementById('audit-records'); var operationRecords = document.getElementById('operation-records');
      ...
      function selectView(name) { tabs.forEach(function (tab) { var selected = tab.dataset.tab === name; tab.setAttribute('aria-selected', String(selected)); }); views.forEach(function (view) { view.hidden = view.dataset.view !== name; }); document.getElementById(name + '-view').focus({ preventScroll: true }); }
      tabs.forEach(function (tab) { tab.addEventListener('click', function () { selectView(tab.dataset.tab); }); });
      ...
      async function api(path, init) { var response = await fetch(path, { method: init && init.method || 'GET', credentials: 'same-origin', headers: { authorization: 'Bearer ' + token.value } }); if (!response.ok) throw new Error('Local API request failed (' + response.status + ')'); return response.json(); }
      ...
      function challengeRecord(item) { var cancel = item.status === 'pending' ? '<button class="quiet cancel-challenge" type="button" data-challenge-id="' + escapeHtml(item.id) + '" aria-label="Cancel pending challenge">Cancel pending challenge</button>' : ''; return '<article class="record"><strong>' + escapeHtml(item.mode + ' · ' + item.provider) + '</strong><p>' + escapeHtml(item.runtime + ' · ' + item.scope) + '</p><span class="pill">' + escapeHtml(item.status) + '</span>' + cancel + '</article>'; }
      function renderWorkspace(data) {
        var challenges = data.challenges && data.challenges.challenges || []; guidedRecords.innerHTML = challenges.length ? challenges.map(challengeRecord).join('') : '<p class="empty">No durable guided-auth challenges are recorded.</p>';
        var scopes = data.scopes && data.scopes.scopes || []; scopeRecords.innerHTML = scopes.length ? scopes.map(function (item) { return record(scopeRecords, item.runtime, item.scope.kind + ':' + item.scope.id, (item.capabilities && item.capabilities.readStatus) ? 'readable' : 'unproven'); }).join('') : '<p class="empty">No readable runtime scopes were reported.</p>';
        var catalog = data.models && data.models.models || []; catalogModels.innerHTML = catalog.length ? catalog.map(function (item) { return record(catalogModels, item.id, item.selectable ? 'Cataloged for observation.' : 'Not selectable in this observed policy.', item.selectable ? 'observed' : item.reason || 'blocked'); }).join('') : '<p class="empty">No observed models were reported.</p>';
        var audit = data.audit && data.audit.records || []; auditRecords.innerHTML = audit.length ? audit.map(function (item) { return record(auditRecords, item.action, item.summary || 'No summary supplied.', item.outcome); }).join('') : '<p class="empty">No Account Center audit records are available.</p>';
        var operations = data.operations && data.operations.operations || []; operationRecords.innerHTML = operations.length ? operations.map(function (item) { return record(operationRecords, item.operationId || 'operation', item.outcome || 'No terminal outcome.', item.state || 'unproven'); }).join('') : '<p class="empty">No completed protected operations are available.</p>';
      }
      async function loadWorkspace() { var results = await Promise.allSettled([api('/api/auth-challenges'), api('/api/scopes'), api('/api/models'), api('/api/audit'), api('/api/mutation-operations')]); var values = results.map(function (result) { return result.status === 'fulfilled' ? result.value : null; }); renderWorkspace({ challenges: values[0], scopes: values[1], models: values[2], audit: values[3], operations: values[4] }); }
      guidedRecords.addEventListener('click', async function (event) { var button = event.target.closest('.cancel-challenge'); if (!button || button.disabled) return; var id = button.dataset.challengeId; if (!id) return; button.disabled = true; button.textContent = 'Cancelling…'; setNotice('Cancelling the pending guided-auth challenge…', 'loading'); try { await api('/api/auth-challenges/' + encodeURIComponent(id) + '/cancel', { method: 'POST' }); await loadWorkspace(); setNotice('Guided-auth challenge cancelled. No credentials were changed.', 'ready'); } catch (_) { button.disabled = false; button.textContent = 'Cancel pending challenge'; setNotice('The guided-auth challenge could not be cancelled. Refresh to verify its state.', 'error'); } });
