...
<body>
  ...
  <script>
    (function () {
      ...
      function isChallengeDetail(challenge) { var fields = ['id', 'mode', 'provider', 'runtime', 'scope', 'status', 'expiresAt', 'createdAt', 'updatedAt']; if (!challenge || typeof challenge !== 'object' || Object.keys(challenge).some(function (key) { return fields.indexOf(key) === -1; })) return false; var scope = challenge.scope; return typeof challenge.id === 'string' && /^auth_[a-f0-9-]{36}$/.test(challenge.id) && (challenge.mode === 'add' || challenge.mode === 'reauth') && typeof challenge.provider === 'string' && typeof challenge.runtime === 'string' && scope && typeof scope === 'object' && typeof scope.kind === 'string' && typeof scope.id === 'string' && ['pending', 'completed', 'failed', 'cancelled', 'expired'].indexOf(challenge.status) !== -1 && typeof challenge.createdAt === 'string' && typeof challenge.updatedAt === 'string' && (!challenge.expiresAt || typeof challenge.expiresAt === 'string'); }
