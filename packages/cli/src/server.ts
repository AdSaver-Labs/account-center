...
<body>
  ...
  <script>
    (function () {
      ...
      function renderRuntimeCoverageExplanation() { var liveOpenClaw = latestStatus && latestStatus.source === 'openclaw'; runtimeCoverageExplanation.textContent = liveOpenClaw ? 'OpenClaw coverage is shown only from protected live OpenClaw evidence. Hermes and Codex remain UNPROVEN unless separately observed.' : latestStatus && latestStatus.source === 'fixture' ? 'Example data from the fixture status; not live Hermes or OpenClaw discovery.' : 'Runtime coverage source is UNPROVEN. Hermes, OpenClaw, and Codex are not shown as live discovery.'; return liveOpenClaw; }
