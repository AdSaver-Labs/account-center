...
interface MutationOperationQuery { limit: number; outcome?: "applied" | "not_applied" | "blocked" | "failed"; cursor?: string; }
...
function mutationOperationQuery(path: string): MutationOperationQuery | undefined {
  ...
  return { limit, ...(outcome === null ? {} : { outcome: outcome as MutationOperationQuery["outcome"] }), ...(cursor === null ? {} : { cursor }) };
}
...
async function mutationOperationHistory(repository: MutationRepository | undefined, query: MutationOperationQuery = { limit: 50 }): Promise<unknown | undefined> {
  const matching = repository ? (await repository.list()).slice().reverse().filter((operation) => !query.outcome || operation.outcome === query.outcome) : [];
...
<body>
  ...
  <main class="shell" id="workspace" tabindex="-1">
    ...
    <section class="view secondary" id="guided-view" data-view="guided" role="tabpanel" aria-labelledby="guided-tab" tabindex="-1" hidden><header class="view-heading"><div><h2>Guided auth</h2><p>Durable local challenge history. Starting or completing authentication is not available until runtime proof exists.</p></div><span class="pill warn" id="guided-freshness">UNPROVEN</span></header><div class="record-list" id="guided-records"><p class="empty">Load status to inspect local challenges.</p></div><section class="record-list secondary" id="guided-detail" aria-live="polite" aria-label="Guided-auth challenge detail"></section></section>
  ...
  <script>
    (function () {
      ...
      async function loadOperations(cursor) { var parameters = new URLSearchParams(); if (operationOutcome.value) parameters.set('outcome', operationOutcome.value); if (cursor) parameters.set('cursor', cursor); var suffix = parameters.toString(); return api('/api/mutation-operations' + (suffix ? '?' + suffix : '')); }
