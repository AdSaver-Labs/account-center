...
<body>
  ...
  <script>
    (function () {
      ...
      async function api(path) { var response = await fetch(path, { headers: { authorization: 'Bearer ' + token.value } }); if (!response.ok) throw new Error(path + ' failed (' + response.status + ')'); return response.json(); }
      ...
      function renderWorkspace(data) {
        var challenges = data.challenges && data.challenges.challenges || []; guidedRecords.innerHTML = challenges.length ? challenges.map(function (item) { return record(guidedRecords, item.mode + ' · ' + item.provider, item.runtime + ' · ' + item.scope, item.status); }).join('') : '<p class="empty">No durable guided-auth challenges are recorded.</p>';
