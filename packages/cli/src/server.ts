...
<body>
  ...
  <script>
    (function () {
      ...
      function operationRecord(item) { var operationId = item && typeof item.operationId === 'string' ? item.operationId : 'Operation ID not reported'; var audit = item && item.audit && typeof item.audit === 'object' ? item.audit : {}; var action = typeof audit.action === 'string' ? audit.action : 'Protected operation'; var outcome = item && typeof item.outcome === 'string' ? item.outcome : 'UNPROVEN'; var state = item && typeof item.state === 'string' ? item.state : 'UNPROVEN'; var completed = item && typeof item.completedAt === 'string' ? item.completedAt : 'Not reported'; var warnings = Array.isArray(audit.warningCodes) ? audit.warningCodes.length : 0; return '<article class="record"><strong>' + escapeHtml(action) + '</strong><p>Operation ID: ' + escapeHtml(operationId) + ' · Completed: ' + escapeHtml(completed) + ' · Warnings: ' + escapeHtml(warnings) + '</p><span class="pill">' + escapeHtml(outcome + ' · ' + state) + '</span></article>'; }
