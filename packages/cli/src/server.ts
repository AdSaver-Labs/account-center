...
<body>
  ...
  <script>
    (function () {
      ...
      async function loadWorkspace() { var results = await Promise.allSettled([api('/api/auth-challenges'), api('/api/scopes'), api('/api/models'), loadAudit(), api('/api/mutation-operations')]); var keys = ['challenges', 'scopes', 'models', 'audit', 'operations']; var values = {}; var unavailable = {}; results.forEach(function (result, index) { var key = keys[index]; var field = key === 'challenges' ? 'challenges' : key === 'scopes' ? 'scopes' : key === 'models' ? 'models' : key === 'audit' ? 'records' : 'operations'; if (result.status === 'fulfilled' && result.value && Array.isArray(result.value[field])) values[key] = result.value; else unavailable[key] = true; }); renderWorkspace(values, unavailable); return Object.keys(unavailable).length > 0; }
