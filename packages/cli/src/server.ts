...
<body>
  ...
  <script>
    (function () {
      ...
      function isAuditInventory(data) { return data && typeof data === 'object' && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'records', 'nextCursor'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.audit-history.v1' && isTimestamp(data.generatedAt) && Array.isArray(data.records) && data.records.every(isAuditDetail) && (data.nextCursor === undefined || typeof data.nextCursor === 'string' && /^audit_[a-f0-9-]{36}$/.test(data.nextCursor)); }
      ...
      function isScopeCatalog(data) { return data && typeof data === 'object' && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'scopes'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.runtime-scopes.v1' && isTimestamp(data.generatedAt) && Array.isArray(data.scopes) && data.scopes.every(isRuntimeScope); }
      ...
      function isChallengeInventory(data) { return data && typeof data === 'object' && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'challenges', 'nextCursor'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.auth-challenges.v1' && isTimestamp(data.generatedAt) && Array.isArray(data.challenges) && data.challenges.every(isChallengeDetail) && (data.nextCursor === undefined || typeof data.nextCursor === 'string' && /^auth_[a-f0-9-]{36}$/.test(data.nextCursor)); }
      ...
      async function loadWorkspace() { var scopeResult = (await Promise.allSettled([api('/api/scopes')]))[0]; var values = {}; var unavailable = {}; if (scopeResult.status === 'fulfilled' && isScopeCatalog(scopeResult.value)) values.scopes = scopeResult.value; else unavailable.scopes = true; renderContextSelector(values.scopes, unavailable.scopes); var results = await Promise.allSettled([api('/api/capabilities'), loadChallenges(), loadModels(), loadLimits(), loadAudit(), loadOperations(), api('/api/agent-connections')]); var keys = ['capabilities', 'challenges', 'models', 'limits', 'audit', 'operations', 'connections']; results.forEach(function (result, index) { var key = keys[index]; var field = key === 'capabilities' ? 'actions' : key === 'challenges' ? 'challenges' : key === 'models' ? 'models' : key === 'limits' ? 'accounts' : key === 'audit' ? 'records' : key === 'operations' ? 'operations' : 'inventory'; var valid = result.status === 'fulfilled' && result.value && Array.isArray(result.value[field]); if (key === 'challenges') valid = valid && isChallengeInventory(result.value); if (key === 'models') valid = valid && isModelCatalog(result.value); if (key === 'limits') valid = valid && isLimitsInventory(result.value); if (key === 'audit') valid = valid && isAuditInventory(result.value); if (key === 'operations') valid = valid && isOperationInventory(result.value); if (key === 'connections') valid = valid && isAgentConnectionInventory(result.value); if (valid) values[key] = result.value; else unavailable[key] = true; }); renderWorkspace(values, unavailable); return Object.keys(unavailable).length > 0; }
