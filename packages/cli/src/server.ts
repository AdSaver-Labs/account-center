...
function mutationOperationQuery(path: string): MutationOperationQuery | undefined {
  const parameters = new URL(path, "http://account-center.local").searchParams;
  if ([...parameters.keys()].some((key) => key !== "limit" && key !== "outcome" && key !== "action" && key !== "runtime" && key !== "scopeKind" && key !== "cursor") || ["limit", "outcome", "action", "runtime", "scopeKind", "cursor"].some((key) => parameters.getAll(key).length > 1)) return undefined;
  const limitValue = parameters.get("limit");
  const limit = limitValue === null ? 50 : Number(limitValue);
  if (!Number.isInteger(limit) || limit < 1 || limit > 100) return undefined;
  const outcome = parameters.get("outcome");
...
<body>
  ...
  <script>
    (function () {
      ...
      async function loadOperations(cursor) { var parameters = new URLSearchParams(); var runtime = selectedRuntime(); var scopeKind = selectedScopeKind(); if (operationOutcome.value) parameters.set('outcome', operationOutcome.value); if (operationAction.value) parameters.set('action', operationAction.value); if (runtime) parameters.set('runtime', runtime); if (scopeKind) parameters.set('scopeKind', scopeKind); if (cursor) parameters.set('cursor', cursor); var suffix = parameters.toString(); return api('/api/mutation-operations' + (suffix ? '?' + suffix : '')); }
