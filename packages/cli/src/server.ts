...
<body>
  ...
  <script>
    (function () {
      ...
      function challengeRecord(item) { var cancel = item.status === 'pending' ? '<button class="quiet cancel-challenge" type="button" data-challenge-id="' + escapeHtml(item.id) + '" aria-label="Cancel pending challenge">Cancel pending challenge</button>' : ''; return '<article class="record"><strong>' + escapeHtml(item.mode + ' · ' + item.provider) + '</strong><p>' + escapeHtml(item.runtime + ' · ' + scopeLabel(item.scope)) + '</p><span class="pill">' + escapeHtml(item.status) + '</span>' + cancel + '</article>'; }
      ...
      form.addEventListener('submit', async function (event) { event.preventDefault(); if (!token.value) { token.focus(); setNotice('A launch token is required to request status.', 'error'); return; } refresh.disabled = true; refresh.textContent = 'Refreshing…'; setNotice('Requesting local runtime status…', 'loading'); try { var status = await api('/api/status'); render(status); var incomplete = await loadWorkspace(); setNotice(incomplete ? 'Workspace refreshed; some evidence is UNPROVEN. Retry unavailable sections.' : 'Local workspace refreshed. Read-only evidence is current.', incomplete ? 'error' : 'ready'); } catch (error) { setNotice(error instanceof Error ? error.message : 'Status request could not be completed.', 'error'); } finally { refresh.disabled = false; refresh.textContent = 'Refresh status'; } });
    }());
  </script>
</body>
</html>`;
