...
export interface AccountCenterServerOptions {
  ...
  /** Test seam for the fixed-command, redacted Hermes gateway liveness view. */
  hermesGatewayLivenessReader?: () => Promise<HermesGatewayLivenessView>;
  ...
  accountUiPreferencesStore?: AccountUiPreferencesStore;
}
...
export function createAccountCenterServer(options: AccountCenterServerOptions) {
  ...
  const readHermesGatewayLiveness = () => Promise.resolve().then(() => options.hermesGatewayLivenessReader ? options.hermesGatewayLivenessReader() : new HermesGatewayLivenessReader().read());
  ...
  const handleRequest = async (request: IncomingMessage, response: ServerResponse) => {
    ...
    if (requestUrl.pathname === "/api/hermes-gateway-liveness") {
      const query = exactRuntimeInventoryQuery(request.url ?? "/");
      if (!query || query.runtime !== "hermes" || query.scope !== "default") return send(response, 400, { error: "invalid_query" });
      try {
        const liveness = await readHermesGatewayLiveness();
        return isHermesGatewayLivenessView(liveness) ? send(response, 200, liveness) : send(response, 503, hermesGatewayLivenessUnavailableView());
      } catch { return send(response, 503, hermesGatewayLivenessUnavailableView()); }
    }
    ...
    const operationId = request.method === "GET" ? mutationOperationId(requestUrl.pathname) : undefined;
    if (operationId) {
      const query = durableDetailQuery(request.url ?? "/");
      if (!query) return send(response, 400, { error: "invalid_query" });
      if (!options.mutationRepository) return send(response, 503, { error: "mutation_operations_unavailable" });
...
interface DurableDetailQuery { runtime: string; scopeKind: "default"; }

async function auditRecordDetail(store: AuditStore | undefined, id: string, query: DurableDetailQuery): Promise<unknown | undefined> {
  const record = store ? (await store.list({ limit: 1_000 })).find((candidate) => candidate.id === id && candidate.runtime === query.runtime && candidate.scopeKind === query.scopeKind) : undefined;
  return record ? {
    schemaVersion: "account-center.audit-record.v1",
    generatedAt: new Date().toISOString(),
    record: auditRecordView(record)
  } : undefined;
}

// Detail reads are tied to the only scope semantics public status currently
// authoritatively exposes: a runtime's default scope. This must stay strict;
// accepting a non-default kind would falsely imply an exact scope binding.
function durableDetailQuery(path: string): DurableDetailQuery | undefined {
  const parameters = new URL(path, "http://account-center.local").searchParams;
  if ([...parameters.keys()].some((key) => key !== "runtime" && key !== "scopeKind") || ["runtime", "scopeKind"].some((key) => parameters.getAll(key).length !== 1)) return undefined;
  const runtime = parameters.get("runtime");
  return runtime && isPublicRuntimeSelector(runtime) && parameters.get("scopeKind") === "default" ? { runtime, scopeKind: "default" } : undefined;
}
...
function exactRuntimeInventoryQuery(path: string): { runtime: string; scope: string } | undefined {
  const query = runtimeInventoryQuery(path);
...
function hermesGatewayLivenessUnavailableView(): HermesGatewayLivenessView {
  return {
    schemaVersion: "account-center.hermes-gateway-liveness.v1", runtime: "hermes", scope: "default", state: "unproven",
    observedAt: new Date().toISOString(), verificationState: "UNPROVEN",
    evidence: "Gateway service liveness is UNPROVEN; provider/account capacity, inventory, route, and recovery are UNPROVEN."
  };
}

function isHermesGatewayLivenessView(value: unknown): value is HermesGatewayLivenessView {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const view = value as HermesGatewayLivenessView;
  if (Object.keys(view).length !== 7 || !["schemaVersion", "runtime", "scope", "state", "observedAt", "verificationState", "evidence"].every((key) => Object.prototype.hasOwnProperty.call(view, key))) return false;
  if (view.schemaVersion !== "account-center.hermes-gateway-liveness.v1" || view.runtime !== "hermes" || view.scope !== "default" || !["running", "stopped", "unproven"].includes(view.state) || !isCanonicalIsoTimestamp(view.observedAt) || view.verificationState !== "UNPROVEN") return false;
  const prefix = view.state === "running" ? `Gateway service liveness observed ${view.observedAt}` : view.state === "stopped" ? `Gateway service stopped observed ${view.observedAt}` : "Gateway service liveness is UNPROVEN";
  return view.evidence === `${prefix}; provider/account capacity, inventory, route, and recovery are UNPROVEN.`;
}
...
async function mutationOperationHistory(repository: MutationRepository | undefined, query: MutationOperationQuery = { limit: 50 }): Promise<unknown | undefined> {
  const matching = repository ? (await repository.list()).slice().reverse().filter((operation) =>
    ...
    (!query.action || operation.audit.action === query.action) &&
    (!query.runtime || operation.audit.runtime === query.runtime) &&
    (!query.scopeKind || operation.audit.scopeKind === query.scopeKind) &&
    (!query.from || operation.createdAt.slice(0, 10) >= query.from) &&
    (!query.to || operation.createdAt.slice(0, 10) <= query.to)
  ) : [];
  ...
  const nextCursor = cursorIndex + 1 + query.limit < matching.length ? operations.at(-1)?.operationId ?? null : null;
  return {
    schemaVersion: "account-center.mutation-operations.v1",
    generatedAt: new Date().toISOString(),
    operations,
    // A nullable cursor makes a terminal page explicit while remaining safe for
    // older consumers that treated an omitted cursor as terminal.
    nextCursor
  };
}

async function mutationOperationDetail(repository: MutationRepository | undefined, operationId: string, query: DurableDetailQuery): Promise<unknown | undefined> {
  const operation = repository ? (await repository.list()).find((candidate) => candidate.operationId === operationId && candidate.audit.runtime === query.runtime && candidate.audit.scopeKind === query.scopeKind) : undefined;
  return operation ? {
...
function endpointMethods(path: string | undefined): string[] | undefined {
  ...
  if (["/api/capabilities", "/api/audit", "/api/mutation-operations", "/api/models", "/api/limits", "/api/agent-connections", "/api/routing-pools", "/api/scopes", "/api/status", "/api/hermes-gateway-liveness"].includes(pathname ?? "")) return ["GET"];
...
function hasUnexpectedQuery(url: URL, rawPath: string | undefined, method: string | undefined): boolean {
  ...
  if (url.pathname === "/api/hermes-gateway-liveness") return !exactRuntimeInventoryQuery(rawPath ?? url.pathname);
...
function auditRecordId(path: string | undefined): string | undefined {
  return path?.match(/^\/api\/audit\/(audit_[a-f0-9-]{36})$/)?.[1];
}
...
function agentCapabilities(challengeStoreAvailable: boolean, auditAvailable: boolean, mutationRepositoryAvailable: boolean, preferenceStoreAvailable: boolean): unknown {
  return {
    ...
    // Fixed public meanings let every client render unavailable operations
    // consistently without accepting adapter-supplied diagnostics or claims.
    stateTaxonomy: (["unsupported", "unentitled", "read_only", "unknown", "UNPROVEN"] as const).map((state) => honestOperationState(state)),
    actions: [
      ...
      { id: "agent_connections.list", mode: "read", state: "available", endpoint: { method: "GET", path: "/api/agent-connections" }, requires: ["bearer_token", "explicit_runtime_scope"] },
...
<head>
  ...
  <style>
    .more-basics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:0 0 14px}.more-basics article{border:1px solid var(--line);border-radius:7px;background:var(--panel-2);padding:11px;min-width:0}.more-basics h2{margin:0;font-size:14px}.more-basics p{margin:5px 0 0;color:var(--muted);font-size:12px}.onboarding-dialog{width:min(560px,calc(100vw - 28px))}.onboarding-dialog h2{margin:0}.onboarding-dialog .eyebrow{margin:0 0 8px}.onboarding-step{color:var(--muted);font-size:12px}.onboarding-dialog ul{margin:8px 0;padding-left:20px;color:var(--muted)}.advanced-disclosure{border:1px solid var(--line);border-radius:var(--radius);background:var(--panel);padding:0 14px}.advanced-disclosure summary{display:flex;align-items:center;justify-content:space-between;gap:12px;min-height:52px;cursor:pointer;color:var(--text)}.advanced-disclosure summary strong{font-size:14px}.advanced-disclosure summary span{color:var(--muted);font-size:12px}.advanced-content{padding:2px 0 14px}.advanced-content>.view{margin-top:14px}@media(max-width:640px){.view-heading{align-items:flex-start;flex-direction:column}.more-basics{grid-template-columns:1fr}.advanced-disclosure summary{align-items:flex-start;flex-direction:column;justify-content:center}.advanced-disclosure summary span{padding-left:20px}}
  </style>
</head>
<body>
  ...
  <main class="shell" id="workspace" tabindex="-1">
    ...
    <nav class="tabs" aria-label="Account Center views" role="tablist"><button class="tab" type="button" role="tab" tabindex="0" aria-selected="true" aria-controls="home-view" id="home-tab" data-tab="home">Home</button><button class="tab" type="button" role="tab" tabindex="-1" aria-selected="false" aria-controls="accounts-view" id="accounts-tab" data-tab="accounts">Accounts</button><button class="tab" type="button" role="tab" tabindex="-1" aria-selected="false" aria-controls="more-view" id="more-tab" data-tab="more">More</button></nav>
    <section class="dashboard view" id="home-view" data-view="home" role="tabpanel" aria-labelledby="home-tab" tabindex="-1">
      ...
      <article class="panel"><div class="panel-header"><div class="section-title"><h2>Runtime coverage</h2><span class="count">Read-only</span></div><span class="pill warn">Evidence gated</span></div><div class="panel-body"><p class="subtle" id="runtime-coverage-explanation" aria-live="polite" aria-atomic="true">Runtime coverage source is UNPROVEN. Hermes, OpenClaw, and Codex are not shown as live discovery.</p><div class="account-records" id="sentinel-runtimes" role="list" aria-label="Runtime coverage"><p class="empty">Load status to inspect runtime coverage.</p></div></div></article>
  ...
  <script>
    (function () {
      ...
      function renderOnboarding() { var step = onboardingSteps[onboardingIndex]; onboardingStep.textContent = 'Step ' + (onboardingIndex + 1) + ' of ' + onboardingSteps.length; onboardingHeading.textContent = step.title; onboardingDescription.textContent = step.detail; onboardingNext.textContent = onboardingIndex === onboardingSteps.length - 1 ? 'Finish' : 'Continue'; }
      function openOnboarding() { onboardingIndex = 0; renderOnboarding(); if (!onboardingDialog.open) onboardingDialog.showModal(); onboardingHeading.focus(); }
      function closeOnboarding() { if (onboardingDialog.open) onboardingDialog.close(); }
      onboardingNext.addEventListener('click', function () { if (onboardingIndex === onboardingSteps.length - 1) { closeOnboarding(); token.focus(); return; } onboardingIndex++; renderOnboarding(); onboardingHeading.focus(); });
      onboardingSkip.addEventListener('click', function () { closeOnboarding(); token.focus(); });
      onboardingDialog.addEventListener('cancel', function (event) { event.preventDefault(); closeOnboarding(); token.focus(); });
      replayOnboarding.addEventListener('click', openOnboarding);
      ...
      // A preference mutation is enabled only by one complete, exact declaration.
      // Do not let a first matching entry hide a duplicate or malformed entry.
      function exactMutationCapability(capabilityData, id, path, requirements) { var actions = capabilityData && Array.isArray(capabilityData.actions) ? capabilityData.actions : []; var matches = actions.filter(function (action) { return action && typeof action === 'object' && action.id === id; }); if (matches.length !== 1) return undefined; var action = matches[0]; if (Object.getPrototypeOf(action) !== Object.prototype || Object.keys(action).length !== 5 || !['id', 'mode', 'state', 'endpoint', 'requires'].every(function (key) { return Object.prototype.hasOwnProperty.call(action, key); }) || action.mode !== 'mutation' || action.state !== 'available' || !action.endpoint || Object.getPrototypeOf(action.endpoint) !== Object.prototype || Object.keys(action.endpoint).length !== 2 || action.endpoint.method !== 'POST' || action.endpoint.path !== path || !Array.isArray(action.requires) || action.requires.length !== requirements.length || !requirements.every(function (requirement, index) { return action.requires[index] === requirement; })) return undefined; return action; }
      // State-changing controls require one complete, exact capability declaration.
      // Never let a first matching entry make an ambiguous manifest actionable.
      function accountUiPreferencesMutationCapability(capabilityData) { return exactMutationCapability(capabilityData, 'account_ui_preferences.mutate', '/api/account-ui-preferences', ['bearer_token', 'same_origin', 'explicit_runtime_scope', 'durable_account_ui_preferences_store']); }
      function authChallengeCancellationCapability(capabilityData) { return exactMutationCapability(capabilityData, 'auth_challenges.cancel', '/api/auth-challenges/:id/cancel?runtime=:runtime&scope=:scope', ['bearer_token', 'same_origin', 'opaque_challenge_id', 'explicit_runtime_scope', 'durable_challenge_store', 'durable_audit_store']); }
      function actionReason(action, fallback) { return action && action.reason ? action.reason.replaceAll('_', ' ') : fallback; }
      function renderAccountsRouting(limitData, capabilityData, unavailable) { var runtime = selectedRuntime(); var route = latestStatus && Array.isArray(latestStatus.routes) ? latestStatus.routes.filter(function (item) { return item.runtime === runtime; })[0] : undefined; var scopedAccounts = limitData && Array.isArray(limitData.accounts) ? limitData.accounts.filter(function (item) { return item && typeof item.accountRef === 'string' && typeof item.provider === 'string' && typeof item.health === 'string' && typeof item.authState === 'string' && typeof item.readable === 'boolean'; }) : []; var routeAction = actionById(capabilityData, 'routes'); if (!latestStatus) { accountsRoutingBadge.textContent = 'Loading'; renderViewState(routingRouteState, 'loading', 'Loading Accounts & routing', 'Load protected runtime status to inspect the selected context.'); renderViewState(routingActionState, 'loading', 'Checking route capability', 'No route action is available until capability discovery completes.'); renderViewState(routingAccountsState, 'loading', 'Loading connected accounts', 'Account status has not been requested.'); return; } accountsRoutingBadge.textContent = runtime ? 'Observed ' + runtime : 'UNPROVEN'; if (!runtime) renderViewState(routingRouteState, 'unproven', 'Selected context is unavailable', 'No readable runtime scope was supplied by the protected API.', 'Retry workspace data'); else if (!route) renderViewState(routingRouteState, 'empty', 'No route reported', 'The protected status response reports no route for ' + runtime + '. This does not prove routing is disabled.'); else routingRouteState.innerHTML = record(routingRouteState, route.provider + ' → ' + route.runtime, 'Active: ' + (route.activeProfileId || 'Not reported') + ' · ' + (route.order || []).length + ' account(s) in API-reported order.', 'read-only'); if (unavailable.capabilities) renderViewState(routingActionState, 'unproven', 'Route capability unavailable', 'The protected capability response could not be verified.', 'Retry workspace data'); else if (!routeAction) renderViewState(routingActionState, 'unproven', 'Route capability not reported', 'No protected route operation contract was supplied.'); else if (routeAction.state === 'available') renderViewState(routingActionState, 'read-only', 'Route mutation is not rendered here', 'A capability exists, but this UI slice exposes no route mutation.'); else renderViewState(routingActionState, routeAction.state === 'blocked' ? 'blocked' : 'unproven', 'Route changes unavailable', actionReason(routeAction, 'The selected route cannot be changed safely.')); if (unavailable.limits) renderViewState(routingAccountsState, 'unproven', 'Selected-scope limit inventory unavailable', 'Selected-scope limit inventory could not be verified. Refresh before treating account evidence as current.', 'Retry workspace data'); else if (!scopedAccounts.length) renderViewState(routingAccountsState, 'empty', 'No connected accounts reported', 'No connected accounts were reported by the protected selected-scope inventory.'); else routingAccountsState.innerHTML = scopedAccounts.map(function (account) { return record(routingAccountsState, account.accountRef, 'Provider: ' + account.provider + ' · Auth: ' + account.authState + ' · Health: ' + account.health + '. ' + (account.readable ? 'Observed readable status.' : 'UNPROVEN — unreadable status.'), account.readable ? 'read-only' : 'UNPROVEN'); }).join(''); }
      ...
      function isSafeRuntimeKey(value) { return typeof value === 'string' && /^[a-z][a-z0-9._-]{0,63}$/.test(value); }
      function isLimitsInventory(data) { return data && typeof data === 'object' && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'accounts'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.limits.v1' && isCanonicalTimestamp(data.generatedAt) && Array.isArray(data.accounts) && new Set(data.accounts.map(function (account) { return account && account.accountRef; })).size === data.accounts.length && data.accounts.every(function (account) { return account && typeof account === 'object' && Object.keys(account).every(function (key) { return ['accountRef', 'provider', 'health', 'authState', 'readable', 'windows'].indexOf(key) !== -1; }) && /^account-[1-9][0-9]*$/.test(account.accountRef) && isSafeRuntimeKey(account.provider) && ['ok', 'warn', 'error', 'unknown'].indexOf(account.health) !== -1 && isSafeRuntimeKey(account.authState) && typeof account.readable === 'boolean' && Array.isArray(account.windows) && new Set(account.windows.map(function (window) { return window && window.name; })).size === account.windows.length && new Set(account.windows.map(function (window) { return window && window.resetsAt; }).filter(function (resetsAt) { return typeof resetsAt === 'string'; })).size === account.windows.filter(function (window) { return window && typeof window.resetsAt === 'string'; }).length && account.windows.every(function (window) { return window && typeof window === 'object' && Object.keys(window).every(function (key) { return ['name', 'remainingPct', 'resetsAt'].indexOf(key) !== -1; }) && isSafeRuntimeKey(window.name) && (window.remainingPct === null || typeof window.remainingPct === 'number' && Number.isFinite(window.remainingPct) && window.remainingPct >= 0 && window.remainingPct <= 100) && (window.resetsAt === undefined || isCanonicalTimestamp(window.resetsAt)); }); }); }
      function isAccountUiPreferences(data) { return data && typeof data === 'object' && Object.keys(data).length === 2 && data.schemaVersion === 'account-center.account-ui-preferences.v1' && Array.isArray(data.hiddenAccountRefs) && new Set(data.hiddenAccountRefs).size === data.hiddenAccountRefs.length && data.hiddenAccountRefs.every(function (ref) { return typeof ref === 'string' && /^account-[1-9][0-9]*$/.test(ref); }); }
      function renderAccountVisibility(limitData, preferenceData, unavailable, preferenceMutation) { if (unavailable || !isLimitsInventory(limitData) || !isAccountUiPreferences(preferenceData)) { renderViewState(accountVisibilityState, 'unproven', 'Account visibility could not be verified', 'The protected account inventory or local redacted preference could not be verified. No account was hidden or removed.', 'Retry workspace data'); return; } if (!preferenceMutation) { renderViewState(accountVisibilityState, 'unproven', 'Account visibility controls unavailable', 'The protected account visibility action could not be verified. No account was hidden or restored.', 'Retry workspace data'); return; } var hidden = preferenceData.hiddenAccountRefs; var runtime = selectedRuntime(); var scope = selectedContext ? selectedContext.split('|').slice(1).join('|') : ''; if (!runtime || !scope) { renderViewState(accountVisibilityState, 'unproven', 'Account visibility scope could not be verified', 'The selected runtime scope is unavailable. No account visibility control is shown.', 'Retry workspace data'); return; } var scopeNotice = '<p class="scope-notice">Changes stay in ' + escapeHtml(runtime.charAt(0).toUpperCase() + runtime.slice(1) + ' / ' + scope) + '. Other runtimes and scopes are unchanged.</p>'; var route = latestStatus && Array.isArray(latestStatus.routes) ? latestStatus.routes.filter(function (item) { return item.runtime === runtime; })[0] : undefined; var profiles = latestStatus && Array.isArray(latestStatus.profiles) ? latestStatus.profiles : []; accountVisibilityState.innerHTML = scopeNotice + (limitData.accounts.map(function (account) { var index = Number(account.accountRef.slice(8)) - 1; var profile = profiles[index]; var active = !!route && profile && route.activeProfileId === profile.id; var state = hidden.indexOf(account.accountRef) !== -1 ? 'hidden' : active ? 'active' : 'saved'; var availability = account.windows[0] && typeof account.windows[0].remainingPct === 'number' ? account.windows[0].remainingPct + '% weekly remaining' : 'UNPROVEN — weekly availability unavailable'; var stateLabel = state.charAt(0).toUpperCase() + state.slice(1); var action = state === 'hidden' ? '<button class="quiet restore-account" type="button" data-account-ref="' + escapeHtml(account.accountRef) + '" aria-label="Restore account to everyday lists; routing and credentials stay unchanged">Restore</button>' : '<button class="quiet hide-account" type="button" data-account-ref="' + escapeHtml(account.accountRef) + '" aria-label="Hide account locally; credentials stay connected and routing stays unchanged">Hide</button>'; return '<article class="record"><strong>' + escapeHtml(account.accountRef) + ' · ' + escapeHtml(stateLabel) + '</strong><p>' + escapeHtml(account.provider) + ' · ' + escapeHtml(account.authState) + ' · ' + escapeHtml(availability) + '</p><p>' + (state === 'hidden' ? 'Hidden locally only. It can be restored to everyday lists; the connected credential and runtime state are preserved.' : state === 'active' ? 'Used by this route now. Route changes remain capability-gated.' : 'Available locally, but not used by this route. It stays connected locally; routing changes remain capability-gated.') + '</p><span class="pill">' + escapeHtml(stateLabel) + '</span>' + action + '</article>'; }).join('') || '<p class="empty">No connected accounts were reported by the protected selected-scope inventory.</p>'); }
      function isModelSelectionEvidence(selection) { return selection && typeof selection === 'object' && Object.keys(selection).length === 4 && Object.keys(selection).every(function (key) { return ['requestedPolicy', 'effectiveRuntimeModel', 'fallbackChain', 'verificationState'].indexOf(key) !== -1; }) && selection.requestedPolicy && selection.effectiveRuntimeModel && selection.fallbackChain && Object.keys(selection.requestedPolicy).length === 1 && Object.keys(selection.effectiveRuntimeModel).length === 1 && Object.keys(selection.fallbackChain).length === 1 && selection.requestedPolicy.state === 'not_reported' && selection.effectiveRuntimeModel.state === 'not_reported' && selection.fallbackChain.state === 'not_reported' && selection.verificationState === 'UNPROVEN'; }
      function isModelCatalog(data) { return data && typeof data === 'object' && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'selection', 'models'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.models.v1' && isCanonicalTimestamp(data.generatedAt) && isModelSelectionEvidence(data.selection) && Array.isArray(data.models) && data.models.every(function (model) { return model && typeof model === 'object' && Object.keys(model).every(function (key) { return ['id', 'selectable', 'reason', 'observedProfileCount', 'readableProfileCount', 'runtimeCompatibility', 'verificationState'].indexOf(key) !== -1; }) && typeof model.id === 'string' && /^[a-z0-9][a-z0-9._/-]{0,191}$/.test(model.id) && typeof model.selectable === 'boolean' && (model.reason === undefined || typeof model.reason === 'string' && /^[a-z][a-z0-9_]{0,79}$/.test(model.reason)) && Number.isInteger(model.observedProfileCount) && model.observedProfileCount >= 0 && Number.isInteger(model.readableProfileCount) && model.readableProfileCount >= 0 && model.readableProfileCount <= model.observedProfileCount && Array.isArray(model.runtimeCompatibility) && model.runtimeCompatibility.every(isSafeRuntimeKey) && model.verificationState === 'UNPROVEN'; }); }
      function modelSelectionEvidence(selection) { var absent = { requestedPolicy: { state: 'not_reported' }, effectiveRuntimeModel: { state: 'not_reported' }, fallbackChain: { state: 'not_reported' }, verificationState: 'UNPROVEN' }; if (!isModelSelectionEvidence(selection)) return absent; return { requestedPolicy: selection.requestedPolicy, effectiveRuntimeModel: selection.effectiveRuntimeModel, fallbackChain: selection.fallbackChain, verificationState: 'UNPROVEN' }; }
      function renderModelEvidenceState(target, state, title, detail, actionLabel) { var tone = state === 'read-only' ? '' : 'warn'; var badge = state === 'read-only' ? 'Read-only evidence' : 'Status unavailable'; target.innerHTML = '<article class="record state" data-ui-state="' + state + '" role="status"><strong>' + escapeHtml(title) + '</strong><p>' + escapeHtml(detail) + '</p><span class="pill ' + tone + '">' + badge + '</span>' + (actionLabel ? '<button class="quiet retry-workspace" type="button">' + escapeHtml(actionLabel) + '</button>' : '') + '</article>'; }
      function renderModelsFallbacks(modelData, unavailable) { var runtime = selectedRuntime(); var catalog = modelData && Array.isArray(modelData.models) ? modelData.models : []; var detail = document.getElementById('models-fallbacks-detail'); if (unavailable) { modelsFallbacksBadge.textContent = 'Status unavailable'; modelsFallbacksBadge.setAttribute('aria-label', 'Model-policy status unavailable'); modelsFallbacksBadge.className = 'pill warn'; detail.textContent = 'Model-policy evidence could not be verified. No model setting or fallback is shown as current.'; renderModelEvidenceState(modelPolicyState, 'unproven', 'Model-policy evidence unavailable', 'The protected model catalog could not be verified. No current model setting is shown.', 'Retry workspace data'); renderModelEvidenceState(modelActionState, 'unproven', 'Model-policy changes unavailable', 'No protected scoped model change can be offered until its safety contract and runtime proof exist.'); modelCatalogState.innerHTML = '<article class="record state" data-ui-state="unproven" role="status"><strong>Model catalog unavailable</strong><p>The protected model catalog could not be verified. No model is shown as selectable.</p><span class="pill warn">Status unavailable</span></article>'; return; } modelsFallbacksBadge.textContent = 'Read-only evidence'; modelsFallbacksBadge.setAttribute('aria-label', 'Read-only model-policy evidence'); modelsFallbacksBadge.className = 'pill'; detail.textContent = 'The model catalog was checked for ' + (runtime || 'the selected runtime') + '. It does not show a currently applied model setting or enable changes.'; renderModelEvidenceState(modelPolicyState, 'read-only', 'Current model selection is not reported', 'Requested policy, effective runtime model, and fallback chain were not reported. The observed catalog does not establish an applied runtime configuration.'); renderModelEvidenceState(modelActionState, 'unproven', 'Model-policy changes unavailable', 'Model selection and fallback changes are unavailable until a protected scoped mutation contract and runtime proof exist.'); modelCatalogState.innerHTML = catalog.length ? catalog.map(function (item) { var state = item.selectable ? 'Not verified' : item.reason ? item.reason.replaceAll('_', ' ') : 'Unavailable'; var compatibility = Array.isArray(item.runtimeCompatibility) && item.runtimeCompatibility.length ? item.runtimeCompatibility.join(', ') : 'Not reported'; return record(modelCatalogState, item.id, 'Compatible runtimes: ' + compatibility + '. Selection is not verified.', state); }).join('') : '<p class="empty">No observed models were reported for this runtime.</p>'; }
      ...
      async function api(path, init) { var headers = { authorization: 'Bearer ' + token.value }; if (init && init.body) headers['content-type'] = 'application/json'; var response = await fetch(path, { method: init && init.method || 'GET', credentials: 'same-origin', headers: headers, body: init && init.body }); if (!response.ok) { var error = new Error(response.status === 401 ? 'Launch token rejected' : 'Local API request failed (' + response.status + ')'); error.status = response.status; throw error; } return response.json(); }
      ...
      function auditRecord(item) { var action = item && typeof item.action === 'string' ? item.action : 'Audit action not reported'; var summary = item && typeof item.summary === 'string' ? item.summary : 'No summary supplied.'; var outcome = item && typeof item.outcome === 'string' ? item.outcome : 'UNPROVEN'; var proof = item && typeof item.proofState === 'string' ? item.proofState : 'UNPROVEN'; var recorded = item && typeof item.createdAt === 'string' ? item.createdAt : 'Not reported'; var auditId = item && typeof item.id === 'string' ? item.id : 'Not reported'; var runtime = item && typeof item.runtime === 'string' ? item.runtime : 'Not reported'; var scopeKind = item && typeof item.scopeKind === 'string' ? item.scopeKind : 'Not reported'; var warnings = item && Array.isArray(item.warnings) ? item.warnings.length : 0; var detail = /^audit_[a-f0-9-]{36}$/.test(auditId) ? '<button class="quiet view-audit" type="button" data-audit-id="' + escapeHtml(auditId) + '" aria-label="View audit evidence">View audit evidence</button>' : ''; return '<article class="record"><strong>' + escapeHtml(action) + '</strong><p>' + escapeHtml(summary) + '</p><p>Runtime: ' + escapeHtml(runtime) + ' · Scope: ' + escapeHtml(scopeKind) + '</p><p>Recorded: ' + escapeHtml(recorded) + ' · Proof: ' + escapeHtml(proof) + ' · Warnings: ' + escapeHtml(warnings) + ' · Audit ID: ' + escapeHtml(auditId) + '</p><span class="pill">' + escapeHtml(outcome) + '</span>' + detail + '</article>'; }
      function isAuditDetail(record) { return record && typeof record === 'object' && Object.keys(record).every(function (key) { return ['id', 'createdAt', 'action', 'outcome', 'proofState', 'summary', 'warnings', 'runtime', 'scopeKind'].indexOf(key) !== -1; }) && /^audit_[a-f0-9-]{36}$/.test(record.id) && isTimestamp(record.createdAt) && /^[a-z][a-z0-9._-]{0,63}$/.test(record.action) && ['dry_run', 'started', 'applied', 'blocked', 'failed_no_change_verified', 'unproven', 'recovery_required'].indexOf(record.outcome) !== -1 && ['verified', 'unproven', 'not_applicable'].indexOf(record.proofState) !== -1 && typeof record.summary === 'string' && Array.isArray(record.warnings) && record.warnings.every(function (warning) { return typeof warning === 'string'; }) && (record.runtime === undefined || /^[a-z][a-z0-9._-]{0,63}$/.test(record.runtime)) && (record.scopeKind === undefined || ['agent', 'profile', 'session', 'default', 'all'].indexOf(record.scopeKind) !== -1); }
      function isAuditInventory(data) { return data && typeof data === 'object' && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'records', 'nextCursor'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.audit-history.v1' && isCanonicalTimestamp(data.generatedAt) && Array.isArray(data.records) && data.records.every(isAuditDetail) && (data.nextCursor === undefined || typeof data.nextCursor === 'string' && /^audit_[a-f0-9-]{36}$/.test(data.nextCursor)); }
      function clearAuditDetail() { auditDetail.innerHTML = '<p class="empty" role="status">No audit evidence detail selected for this context.</p>'; }
      function renderAuditDetail(record) { if (!isAuditDetail(record)) { auditDetail.innerHTML = '<article class="record state" data-ui-state="unproven" role="status"><strong>UNPROVEN — data unavailable</strong><p>Audit evidence detail was malformed or incomplete. Refresh audit history and try again.</p><span class="pill warn">UNPROVEN</span></article>'; return; } auditDetail.innerHTML = '<article class="record"><strong>Audit evidence detail</strong><p>' + escapeHtml(record.summary) + '</p><p>Action: ' + escapeHtml(record.action) + ' · Outcome: ' + escapeHtml(record.outcome) + ' · Proof: ' + escapeHtml(record.proofState) + '</p><p>Runtime: ' + escapeHtml(record.runtime || 'Not reported') + ' · Scope: ' + escapeHtml(record.scopeKind || 'Not reported') + ' · Recorded: ' + escapeHtml(record.createdAt) + '</p><p>Warnings: ' + escapeHtml(record.warnings.length ? record.warnings.join(', ') : 'None') + '. This redacted evidence does not include request digests, targets, scope IDs, or runtime configuration.</p><span class="pill">' + escapeHtml(record.outcome) + '</span></article>'; }
      ...
      function isOperationView(operation) { var audit = operation && operation.audit; var completed = operation && operation.state === 'completed'; return operation && typeof operation === 'object' && Object.keys(operation).every(function (key) { return ['operationId', 'state', 'outcome', 'createdAt', 'completedAt', 'audit'].indexOf(key) !== -1; }) && /^op_[A-Za-z0-9_-]{1,100}$/.test(operation.operationId) && (operation.state === 'pending' || completed) && isCanonicalTimestamp(operation.createdAt) && (!completed || ['applied', 'not_applied', 'blocked', 'failed'].indexOf(operation.outcome) !== -1 && isCanonicalTimestamp(operation.completedAt)) && (!completed || operation.outcome !== undefined && operation.completedAt !== undefined) && (completed || operation.outcome === undefined && operation.completedAt === undefined) && audit && typeof audit === 'object' && Object.keys(audit).every(function (key) { return ['action', 'provider', 'runtime', 'scopeKind', 'warningCodes'].indexOf(key) !== -1; }) && /^[a-z][a-z0-9._-]{0,63}$/.test(audit.action) && /^[a-z][a-z0-9._-]{0,63}$/.test(audit.provider) && /^[a-z][a-z0-9._-]{0,63}$/.test(audit.runtime) && ['agent', 'profile', 'session', 'default', 'all'].indexOf(audit.scopeKind) !== -1 && Array.isArray(audit.warningCodes) && audit.warningCodes.every(function (warning) { return typeof warning === 'string' && /^[a-z][a-z0-9_]{0,79}$/.test(warning); }); }
      function isOperationInventory(data) { return data && typeof data === 'object' && Object.prototype.hasOwnProperty.call(data, 'nextCursor') && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'operations', 'nextCursor'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.mutation-operations.v1' && isCanonicalTimestamp(data.generatedAt) && Array.isArray(data.operations) && data.operations.every(isOperationView) && (data.nextCursor === null || typeof data.nextCursor === 'string' && /^op_[A-Za-z0-9_-]{1,100}$/.test(data.nextCursor)); }
      function isOperationDetail(operation) { return isOperationView(operation) && operation.state === 'completed' && ['applied', 'not_applied', 'blocked', 'failed'].indexOf(operation.outcome) !== -1 && isCanonicalTimestamp(operation.completedAt); }
      ...
      function clearOperationDetail() { operationDetail.innerHTML = '<p class="empty" role="status">No protected operation detail selected for this context.</p>'; }
      function clearChallengeDetail() { guidedDetail.innerHTML = '<p class="empty" role="status">No guided-auth challenge detail selected for this context.</p>'; }
      function scopeLabel(scope) { return scope && typeof scope === 'object' && typeof scope.kind === 'string' && typeof scope.id === 'string' ? (scope.kind === 'default' && scope.id === 'default' ? 'default' : scope.kind + ':' + scope.id) : typeof scope === 'string' ? scope : 'scope not reported'; }
      ...
      function challengeRecord(item, cancellation) { var canCancel = item.status === 'pending' && cancellation && cancellation.state === 'available'; var cancel = canCancel ? '<button class="quiet cancel-challenge" type="button" data-challenge-id="' + escapeHtml(item.id) + '" aria-label="Cancel pending challenge">Cancel pending challenge</button>' : ''; var cancellationState = item.status === 'pending' && !canCancel ? '<p>Cancellation is ' + escapeHtml(cancellation && cancellation.state === 'blocked' ? 'blocked: ' + actionReason(cancellation, 'durable lifecycle evidence is unavailable') : 'UNPROVEN: protected capability discovery is unavailable') + '.</p>' : ''; return '<article class="record"><strong>' + escapeHtml(item.mode + ' · ' + item.provider) + '</strong><p>' + escapeHtml(item.runtime + ' · ' + scopeLabel(item.scope)) + '</p><span class="pill">' + escapeHtml(item.status) + '</span><button class="quiet view-challenge" type="button" data-challenge-id="' + escapeHtml(item.id) + '" aria-label="View challenge details">View challenge details</button>' + cancellationState + cancel + '</article>'; }
      function isTimestamp(value) { return typeof value === 'string' && !Number.isNaN(Date.parse(value)); }
      function isCanonicalTimestamp(value) { return isTimestamp(value) && new Date(value).toISOString() === value; }
      ...
      function renderOperatorActions(capabilityData, unavailable) { var names = { routes: 'Switch route', guided_auth: 'Start guided auth', models: 'Change model policy', 'account.delete': 'Credential deletion (deferred)', updates: 'Update Account Center' }; if (unavailable) { operatorActions.innerHTML = unavailableRecord('Action capability contract'); return; } var actions = capabilityData && capabilityData.actions || []; var taxonomy = capabilityData && Array.isArray(capabilityData.stateTaxonomy) ? capabilityData.stateTaxonomy : []; var stateMeaning = function (state) { var item = taxonomy.filter(function (entry) { return entry && entry.state === state && entry.success === false && entry.terminal === true && typeof entry.meaning === 'string'; })[0]; return item && item.meaning; }; operatorActions.innerHTML = Object.keys(names).map(function (id) { var action = actions.filter(function (item) { return item.id === id; })[0]; var state = action && action.state || 'UNPROVEN'; var honest = state === 'available' ? 'read_only' : state === 'blocked' ? 'unsupported' : 'UNPROVEN'; var reason = action && action.reason ? action.reason.replaceAll('_', ' ') : (stateMeaning(honest) || 'No protected action contract was reported.'); return '<div class="action-row"><div><strong>' + escapeHtml(names[id]) + '</strong><p>' + escapeHtml(reason) + '</p></div><button class="quiet" type="button" disabled>' + escapeHtml(honest) + '</button></div>'; }).join(''); }
      function renderLimits(limitData, preferenceData, unavailable) { var runtime = selectedRuntime(); var scope = selectedContext ? selectedContext.split('|').slice(1).join('|') : ''; if (!runtime || !scope) { homeAccountScope.textContent = 'Visible account scope could not be verified. No account list is shown.'; document.getElementById('account-count').textContent = 'UNPROVEN'; accounts.innerHTML = '<p class="empty">Select a readable runtime and scope, then refresh to verify visible accounts.</p>'; return; } if (unavailable || !isLimitsInventory(limitData) || !isAccountUiPreferences(preferenceData)) { homeAccountScope.textContent = 'Visible accounts for ' + runtime + ' / ' + scope + ' could not be verified. This list does not identify an active route.'; document.getElementById('account-count').textContent = 'UNPROVEN'; accounts.innerHTML = '<p class="empty">Visible accounts could not be verified because the protected inventory or local visibility preference is unavailable.</p>'; return; } homeAccountScope.textContent = 'Showing the verified account inventory for ' + runtime + ' / ' + scope + '. This list does not identify an active route.'; var hidden = preferenceData.hiddenAccountRefs; var limitAccounts = limitData.accounts.filter(function (item) { return hidden.indexOf(item.accountRef) === -1; }); var hiddenCount = limitData.accounts.length - limitAccounts.length; document.getElementById('account-count').textContent = !limitAccounts.length && limitData.accounts.length ? '0 accounts visible — all verified accounts are hidden locally' : limitAccounts.length + (limitAccounts.length === 1 ? ' account' : ' accounts') + (hiddenCount ? ' visible — ' + hiddenCount + (hiddenCount === 1 ? ' verified account is hidden locally' : ' verified accounts are hidden locally') : ''); accounts.innerHTML = limitAccounts.length ? limitAccounts.slice(0, 5).map(function (item) { var windowText = item.windows.map(function (window) { return text(window.name) + ': ' + (window.remainingPct == null ? 'unreadable' : text(window.remainingPct) + '% remaining') + (window.resetsAt ? ' · resets ' + text(window.resetsAt) : ''); }).join(' · '); return '<article class="account-record" role="listitem"><div class="account-name">' + escapeHtml(item.accountRef) + '</div><dl class="account-details"><div><dt>Provider</dt><dd>' + escapeHtml(item.provider) + '</dd></div><div><dt>Health / auth</dt><dd>' + escapeHtml(text(item.health)) + ' · ' + escapeHtml(text(item.authState)) + '</dd></div><div><dt>Capacity</dt><dd>' + escapeHtml(windowText || 'Not reported') + '</dd></div><div><dt>Proof</dt><dd>' + (item.readable === true ? 'Observed readable status' : 'UNPROVEN — unreadable status') + '</dd></div></dl></article>'; }).join('') : limitData.accounts.length ? '<p class="empty">Every verified account is hidden only from everyday lists. Restore an account in Accounts to show it here; credentials and routing are unchanged.</p>' : '<p class="empty">No visible account limits were reported by the protected API.</p>'; }
      function isAgentConnectionInventory(data) { return data && typeof data === 'object' && Object.keys(data).every(function (key) { return ['schemaVersion', 'generatedAt', 'inventory'].indexOf(key) !== -1; }) && data.schemaVersion === 'account-center.agent-connections.v1' && isCanonicalTimestamp(data.generatedAt) && Array.isArray(data.inventory) && data.inventory.every(function (connection) { return connection && typeof connection === 'object' && Object.keys(connection).every(function (key) { return ['connectionRef', 'runtime', 'state', 'onboarding', 'accounts'].indexOf(key) !== -1; }) && /^connection-[a-f0-9]{24}$/.test(connection.connectionRef) && ['hermes', 'openclaw'].indexOf(connection.runtime) !== -1 && ['connected', 'needs-auth', 'unavailable'].indexOf(connection.state) !== -1 && connection.onboarding && typeof connection.onboarding === 'object' && Object.keys(connection.onboarding).length === 1 && ['connect-local-adapter', 'reauth-local-adapter'].indexOf(connection.onboarding.action) !== -1 && Array.isArray(connection.accounts) && connection.accounts.every(function (account) { return account && typeof account === 'object' && Object.keys(account).every(function (key) { return ['accountRef', 'state', 'pairing', 'weeklyRemainingPct', 'routeState', 'lease'].indexOf(key) !== -1; }) && /^account-[1-9][0-9]*$/.test(account.accountRef) && ['usable', 'needs-auth', 'unavailable'].indexOf(account.state) !== -1 && ['paired-verified', 'paired-unverified', 'unpaired'].indexOf(account.pairing) !== -1 && (account.weeklyRemainingPct === null || typeof account.weeklyRemainingPct === 'number' && account.weeklyRemainingPct >= 0 && account.weeklyRemainingPct <= 100) && ['selected', 'available', 'not-routed'].indexOf(account.routeState) !== -1 && (!Object.prototype.hasOwnProperty.call(account, 'lease') || isScopedAccountLease(account.lease, connection, account)); }); }); }
      function isScopedAccountLease(lease, connection, account) { return lease && typeof lease === 'object' && Object.keys(lease).every(function (key) { return ['schemaVersion', 'leaseRef', 'connectionRef', 'accountRef', 'runtime', 'state'].indexOf(key) !== -1; }) && Object.keys(lease).length === 6 && lease.schemaVersion === 'account-center.scoped-account-lease.v1' && lease.leaseRef === 'lease-' + connection.connectionRef + '-' + account.accountRef && lease.connectionRef === connection.connectionRef && lease.accountRef === account.accountRef && lease.runtime === connection.runtime && lease.state === 'verified'; }
      function renderAgentConnections(data, unavailable) { if (unavailable || !isAgentConnectionInventory(data)) { renderViewState(agentConnectionState, 'unproven', 'Agent connection inventory unavailable', 'Connection state could not be verified. Do not copy credentials between runtimes.', 'Retry workspace data'); return; } if (!data.inventory.length) { renderViewState(agentConnectionState, 'empty', 'No agent connections reported', 'Select Hermes or OpenClaw and connect its local adapter.'); return; } agentConnectionState.innerHTML = data.inventory.map(function (connection) { var detail = connection.accounts.map(function (account) { return account.accountRef + ' · ' + account.pairing + ' · ' + account.state + ' · Weekly availability: ' + (account.weeklyRemainingPct == null ? 'unreadable' : account.weeklyRemainingPct + '%') + ' · Route: ' + account.routeState + (account.lease ? ' · scoped lease verified' : ''); }).join(' | '); var action = connection.state === 'needs-auth' ? 'Run local reauthentication only; do not copy a token from another runtime.' : 'Pair one displayed account with this exact scope, then verify its local adapter. Unpaired accounts are inventory-only.'; return record(agentConnectionState, connection.runtime + ' · ' + connection.state, detail + '. ' + action, connection.state); }).join(''); }
      function sentinelRuntimeState(runtime, connections) { var connection = connections && Array.isArray(connections.inventory) ? connections.inventory.filter(function (item) { return item.runtime === runtime; })[0] : undefined; if (connection && ['connected', 'needs-auth', 'unavailable'].indexOf(connection.state) !== -1) return connection.state; var observed = latestStatus && Array.isArray(latestStatus.runtimes) && latestStatus.runtimes.some(function (item) { return item.key === runtime && item.capabilities && item.capabilities.readStatus === true; }); return observed ? 'available' : 'unproven'; }
      function isHermesGatewayLiveness(data) { return data && typeof data === 'object' && Object.keys(data).length === 7 && ['schemaVersion', 'runtime', 'scope', 'state', 'observedAt', 'verificationState', 'evidence'].every(function (key) { return Object.prototype.hasOwnProperty.call(data, key); }) && data.schemaVersion === 'account-center.hermes-gateway-liveness.v1' && data.runtime === 'hermes' && data.scope === 'default' && ['running', 'stopped', 'unproven'].indexOf(data.state) !== -1 && isCanonicalTimestamp(data.observedAt) && data.verificationState === 'UNPROVEN' && data.evidence === (data.state === 'running' ? 'Gateway service liveness observed ' + data.observedAt : data.state === 'stopped' ? 'Gateway service stopped observed ' + data.observedAt : 'Gateway service liveness is UNPROVEN') + '; provider/account capacity, inventory, route, and recovery are UNPROVEN.'; }
      function renderRuntimeCoverageExplanation(gateway, announce) { var sourceExplanation = latestStatus && latestStatus.source === 'openclaw' ? 'OpenClaw coverage is shown only from protected OpenClaw status evidence; it may be a recorded snapshot, not a current runtime probe.' : latestStatus && latestStatus.source === 'fixture' ? 'Example data from the fixture status; not live Hermes or OpenClaw discovery.' : 'Runtime coverage source is UNPROVEN. OpenClaw and Codex are not shown as live discovery.'; var evidence = isHermesGatewayLiveness(gateway) ? gateway.evidence + ' ' : ''; if (announce !== false) runtimeCoverageExplanation.textContent = evidence + sourceExplanation + ' Codex remains UNPROVEN.'; return latestStatus && latestStatus.source === 'openclaw'; }
      function renderSentinelRuntimes(connectionData, gateway, unavailable, announce) { var liveOpenClaw = renderRuntimeCoverageExplanation(gateway, announce); var generatedAt = latestStatus && isTimestamp(latestStatus.generatedAt) ? latestStatus.generatedAt : 'Not reported'; sentinelRuntimes.innerHTML = ['hermes', 'openclaw', 'codex'].map(function (runtime) { var hermesEvidence = runtime === 'hermes' && isHermesGatewayLiveness(gateway) ? gateway : undefined; var state = hermesEvidence ? hermesEvidence.state : !liveOpenClaw || unavailable.connections || runtime !== 'openclaw' ? 'unproven' : sentinelRuntimeState(runtime, connectionData); var verified = hermesEvidence ? hermesEvidence.evidence : liveOpenClaw && runtime === 'openclaw' && (state === 'connected' || state === 'available') ? generatedAt : 'UNPROVEN — not verified'; var label = state === 'running' ? 'Running' : state === 'stopped' ? 'Stopped' : state === 'needs-auth' ? 'Needs auth' : state === 'connected' ? 'Connected' : state === 'available' ? 'Available' : state === 'unavailable' ? 'Unavailable' : 'UNPROVEN'; return '<article class="account-record" role="listitem"><div class="account-name">' + escapeHtml(runtime) + '</div><dl class="account-details"><div><dt>State</dt><dd>' + escapeHtml(label) + '</dd></div><div><dt>Evidence</dt><dd>' + escapeHtml(verified) + '</dd></div></dl></article>'; }).join(''); }
      function renderWorkspace(data, unavailable) {
        ...
        renderLimits(data.limits, data.preferences, unavailable.limits || unavailable.preferences);
        renderModelsFallbacks(data.models, unavailable.models);
        renderOperatorActions(data.capabilities, unavailable.capabilities);
        renderAccountsRouting(data.limits, data.capabilities, unavailable);
        var preferenceMutation = unavailable.capabilities ? undefined : accountUiPreferencesMutationCapability(data.capabilities); renderAccountVisibility(data.limits, data.preferences, unavailable.preferences || unavailable.limits, preferenceMutation);
        renderAgentConnections(data.connections, unavailable.connections);
        renderSentinelRuntimes(data.connections, data.gatewayLiveness, unavailable, false);
        ...
        var challenges = data.challenges && data.challenges.challenges || []; cancelChallengeCapability = unavailable.capabilities ? undefined : authChallengeCancellationCapability(data.capabilities); challengeCursor = !unavailable.challenges && data.challenges && typeof data.challenges.nextCursor === 'string' ? data.challenges.nextCursor : ''; guidedLoadMore.hidden = !challengeCursor; guidedRecords.innerHTML = unavailable.challenges ? unavailableRecord('Guided-auth challenge inventory') : challenges.length ? challenges.map(function (item) { return challengeRecord(item, cancelChallengeCapability); }).join('') : '<p class="empty">No durable guided-auth challenges are recorded.</p>';
      ...
      async function loadAuditDetail(id) { var parameters = new URLSearchParams(); if (selectedRuntime()) parameters.set('runtime', selectedRuntime()); parameters.set('scopeKind', 'default'); return api('/api/audit/' + encodeURIComponent(id) + '?' + parameters.toString()); }
      ...
      operationLoadMore.addEventListener('click', async function () { if (!operationCursor || operationLoadMore.disabled) return; operationLoadMore.disabled = true; operationLoadMore.textContent = 'Loading older protected operations…'; setNotice('Loading older protected operations…', 'loading'); try { var data = await loadOperations(operationCursor); if (!isOperationInventory(data)) throw new Error('malformed_operation_history'); operationFreshness(data); var operations = data.operations; operationCursor = typeof data.nextCursor === 'string' ? data.nextCursor : ''; operationLoadMore.hidden = !operationCursor; if (operations.length) operationRecords.insertAdjacentHTML('beforeend', operations.map(operationRecord).join('')); setNotice(operations.length ? 'Older protected operations are current.' : 'No older protected operations are available.', 'ready'); } catch (_) { operationFreshness(undefined); setNotice('Older protected operations could not be verified. Previously loaded evidence is retained.', 'error'); } finally { operationLoadMore.disabled = false; operationLoadMore.textContent = 'Load older protected operations'; } });
      guidedLoadMore.addEventListener('click', async function () { if (!challengeCursor || guidedLoadMore.disabled) return; guidedLoadMore.disabled = true; guidedLoadMore.textContent = 'Loading older challenges…'; setNotice('Loading older guided-auth challenges…', 'loading'); try { var data = await loadChallenges(challengeCursor); if (!isChallengeInventory(data)) throw new Error('malformed_challenge_inventory'); challengeFreshness(data); var challenges = data.challenges; challengeCursor = typeof data.nextCursor === 'string' ? data.nextCursor : ''; guidedLoadMore.hidden = !challengeCursor; if (challenges.length) guidedRecords.insertAdjacentHTML('beforeend', challenges.map(function (item) { return challengeRecord(item, cancelChallengeCapability); }).join('')); setNotice(challenges.length ? 'Older guided-auth challenges are current.' : 'No older guided-auth challenges are available.', 'ready'); } catch (_) { setNotice('Older guided-auth challenges could not be verified. Previously loaded evidence is retained.', 'error'); } finally { guidedLoadMore.disabled = false; guidedLoadMore.textContent = 'Load older guided-auth challenges'; } });
      document.addEventListener('click', async function (event) { var visibility = event.target.closest('.hide-account, .restore-account'); if (visibility) { var accountRef = visibility.dataset.accountRef; if (!accountRef) return; var state = visibility.classList.contains('hide-account') ? 'hidden' : 'active'; var confirmation = state === 'hidden' ? 'Account hidden locally. Credentials and runtime state were preserved; routing was not changed.' : 'Account restored locally to everyday lists. Credentials and runtime state were preserved; routing was not changed.'; visibility.disabled = true; setNotice(state === 'hidden' ? 'Hiding account from the active list…' : 'Restoring account to everyday lists…', 'loading'); try { await api('/api/account-ui-preferences' + selectedScopeQuery(), { method: 'POST', body: JSON.stringify({ accountRef: accountRef, state: state }) }); var incomplete = await loadWorkspace(); setNotice(incomplete ? confirmation + ' Some other workspace evidence is UNPROVEN.' : confirmation, incomplete ? 'error' : 'ready'); } catch (_) { setNotice('Account visibility could not be verified. No credential deletion was requested.', 'error'); } finally { visibility.disabled = false; } return; } var guided = event.target.closest('.view-guided'); if (guided) { selectView('more', false); var guidedView = document.getElementById('guided-view'); guidedView.scrollIntoView({ block: 'start' }); guidedView.focus({ preventScroll: true }); return; } var audit = event.target.closest('.view-audit'); if (audit) { var auditId = audit.dataset.auditId; if (!auditId) return; audit.disabled = true; audit.textContent = 'Loading…'; auditDetail.innerHTML = '<p class="empty" role="status">Loading audit evidence detail…</p>'; try { var auditRequestEpoch = detailEpoch; var auditResponse = await loadAuditDetail(auditId); if (auditRequestEpoch !== detailEpoch) return; renderAuditDetail(auditResponse.record); setNotice('Audit evidence detail is current.', 'ready'); } catch (_) { if (auditRequestEpoch !== detailEpoch) return; auditDetail.innerHTML = '<article class="record" role="status"><strong>UNPROVEN — data unavailable</strong><p>Audit evidence detail could not be verified. Refresh audit history and try again.</p></article>'; setNotice('Audit evidence detail could not be verified.', 'error'); } finally { audit.disabled = false; audit.textContent = 'View audit evidence'; } return; } var operation = event.target.closest('.view-operation'); if (operation) { var operationId = operation.dataset.operationId; if (!operationId) return; operation.disabled = true; operation.textContent = 'Loading…'; operationDetail.innerHTML = '<p class="empty" role="status">Loading protected operation detail…</p>'; try { var operationRequestEpoch = detailEpoch; var operationResponse = await loadOperationDetail(operationId); if (operationRequestEpoch !== detailEpoch) return; renderOperationDetail(operationResponse.operation); setNotice('Protected operation detail is current.', 'ready'); } catch (_) { if (operationRequestEpoch !== detailEpoch) return; operationDetail.innerHTML = '<article class="record" role="status"><strong>UNPROVEN — data unavailable</strong><p>Protected operation detail could not be verified. Refresh the operation history and try again.</p></article>'; setNotice('Protected operation detail could not be verified.', 'error'); } finally { operation.disabled = false; operation.textContent = 'View protected operation details'; } return; } var detail = event.target.closest('.view-challenge'); if (detail) { var detailId = detail.dataset.challengeId; if (!detailId) return; detail.disabled = true; detail.textContent = 'Loading…'; guidedDetail.innerHTML = '<p class="empty" role="status">Loading guided-auth challenge detail…</p>'; try { var detailResponse = await loadChallengeDetail(detailId); renderChallengeDetail(detailResponse.challenge); setNotice('Guided-auth challenge detail is current.', 'ready'); } catch (_) { guidedDetail.innerHTML = '<article class="record" role="status"><strong>UNPROVEN — data unavailable</strong><p>Guided-auth challenge detail could not be verified. Refresh the challenge inventory and try again.</p></article>'; setNotice('Guided-auth challenge detail could not be verified.', 'error'); } finally { detail.disabled = false; detail.textContent = 'View challenge details'; } return; } var retry = event.target.closest('.retry-workspace'); if (retry) { retry.disabled = true; retry.textContent = 'Retrying…'; setNotice('Retrying unavailable workspace data…', 'loading'); var incomplete = await loadWorkspace(); setNotice(incomplete ? 'Workspace refreshed; some evidence is UNPROVEN. Retry unavailable sections.' : 'Local workspace refreshed. Read-only evidence is current.', incomplete ? 'error' : 'ready'); return; } var button = event.target.closest('.cancel-challenge'); if (!button || button.disabled) return; var id = button.dataset.challengeId; if (!id) return; openCancelChallengeDialog(id, button); });
      function render(status) {
        latestStatus = status;
        // Status source is independently protected evidence. Render its
        // fail-closed coverage state immediately instead of withholding it
        // behind slower scoped workspace reads such as pool discovery.
        renderSentinelRuntimes(undefined, undefined, { connections: true });
