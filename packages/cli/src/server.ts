...
<body>
  ...
  <main class="shell" id="workspace" tabindex="-1">
    ...
    <section class="dashboard view" id="home-view" data-view="home" role="tabpanel" aria-labelledby="home-tab" tabindex="-1">
      ...
      <article class="panel"><div class="panel-header"><div class="section-title"><h2>Runtime coverage</h2><span class="count">Read-only</span></div><span class="pill warn">Evidence gated</span></div><div class="panel-body"><p class="subtle">Hermes and OpenClaw are shown only from protected runtime evidence. Codex remains UNPROVEN until it is explicitly observed.</p><div class="account-records" id="sentinel-runtimes" role="list" aria-label="Runtime coverage"><p class="empty">Load status to inspect runtime coverage.</p></div></div></article>
  ...
  <script>
    (function () {
      ...
      function renderSentinelRuntimes(connectionData, unavailable) { var generatedAt = latestStatus && isTimestamp(latestStatus.generatedAt) ? latestStatus.generatedAt : 'Not reported'; sentinelRuntimes.innerHTML = ['hermes', 'openclaw', 'codex'].map(function (runtime) { var state = unavailable.connections && runtime !== 'codex' ? 'unproven' : sentinelRuntimeState(runtime, connectionData); var verified = state === 'connected' || state === 'available' ? generatedAt : 'UNPROVEN — not verified'; var label = state === 'needs-auth' ? 'Needs auth' : state === 'connected' ? 'Connected' : state === 'available' ? 'Available' : state === 'unavailable' ? 'Unavailable' : 'UNPROVEN'; return '<article class="account-record" role="listitem"><div class="account-name">' + escapeHtml(runtime) + '</div><dl class="account-details"><div><dt>State</dt><dd>' + escapeHtml(label) + '</dd></div><div><dt>Evidence</dt><dd>' + escapeHtml(verified) + '</dd></div></dl></article>'; }).join(''); }
