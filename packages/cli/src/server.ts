...
<body>
  ...
  <script>
    (function () {
      ...
      function renderRuntimeCoverageExplanation(announce) { var openClawStatusEvidence = latestStatus && latestStatus.source === 'openclaw'; if (announce !== false) runtimeCoverageExplanation.textContent = openClawStatusEvidence ? 'OpenClaw coverage is shown only from protected OpenClaw status evidence; it may be a recorded snapshot, not a current runtime probe. Hermes and Codex remain UNPROVEN unless separately observed.' : latestStatus && latestStatus.source === 'fixture' ? 'Example data from the fixture status; not live Hermes or OpenClaw discovery.' : 'Runtime coverage source is UNPROVEN. Hermes, OpenClaw, and Codex are not shown as live discovery.'; return openClawStatusEvidence; }
      function renderSentinelRuntimes(connectionData, unavailable, announce) { var liveOpenClaw = renderRuntimeCoverageExplanation(announce); var generatedAt = latestStatus && isTimestamp(latestStatus.generatedAt) ? latestStatus.generatedAt : 'Not reported'; sentinelRuntimes.innerHTML = ['hermes', 'openclaw', 'codex'].map(function (runtime) { var state = !liveOpenClaw || unavailable.connections || runtime !== 'openclaw' ? 'unproven' : sentinelRuntimeState(runtime, connectionData); var verified = liveOpenClaw && runtime === 'openclaw' && (state === 'connected' || state === 'available') ? generatedAt : 'UNPROVEN — not verified'; var label = state === 'needs-auth' ? 'Needs auth' : state === 'connected' ? 'Connected' : state === 'available' ? 'Available' : state === 'unavailable' ? 'Unavailable' : 'UNPROVEN'; return '<article class="account-record" role="listitem"><div class="account-name">' + escapeHtml(runtime) + '</div><dl class="account-details"><div><dt>State</dt><dd>' + escapeHtml(label) + '</dd></div><div><dt>Evidence</dt><dd>' + escapeHtml(verified) + '</dd></div></dl></article>'; }).join(''); }
      function renderWorkspace(data, unavailable) {
        ...
        renderSentinelRuntimes(data.connections, unavailable, false);
      ...
      function render(status) {
        ...
        renderSentinelRuntimes(undefined, { connections: true });
