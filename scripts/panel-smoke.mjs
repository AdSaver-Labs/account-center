...
try {
  ...
  const [page, status, preferences] = await Promise.all([
    ...
    fetch(`${base}/api/account-ui-preferences?runtime=hermes&scope=default`, { headers })
  ]);
  const html = await page.text();
  const statusBody = await status.json();
  const preferenceBody = await preferences.json();
  if (page.status !== 200 || !html.includes("Account Center") || !html.includes("Account visibility") || status.status !== 200 || !Array.isArray(statusBody.runtimes) || preferences.status !== 200 || preferenceBody.schemaVersion !== "account-center.account-ui-preferences.v1" || !Array.isArray(preferenceBody.hiddenAccountRefs)) throw new Error("panel_smoke_failed");
