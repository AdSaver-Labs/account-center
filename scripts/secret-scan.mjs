...
const patterns = [
  ...
  [/\bBearer\s+[A-Za-z0-9._~+/-]{24,}\b/gi, "bearer token"],
];
...
const { findings, trackedCount } = scanTrackedFiles();
if (findings.length) {
  console.error("Secret scan failed:");
  for (const finding of findings) console.error(`- ${finding}`);
  process.exit(1);
}
console.log(`Secret scan passed: ${trackedCount} tracked files scanned.`);
