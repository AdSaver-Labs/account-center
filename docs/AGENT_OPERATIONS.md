...
| API | `GET /api/scopes` | Available for a read-only runtime scope catalog | Bearer token required; response is `account-center.runtime-scopes.v1` and contains only observed runtime keys, their supported default scope, and declared capabilities. Named agent/profile/session and `all` scopes are unavailable until authoritative runtime evidence exists. |
