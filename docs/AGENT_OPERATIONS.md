...
| API | `GET /api/auth-challenges` | Available for local challenge inventory only | Bearer token required; response is `account-center.auth-challenges.v1` and contains only redacted lifecycle metadata plus an optional expiry timestamp. An omitted runtime/scope is an explicitly aggregate local-history read. A supplied runtime and scope must exactly match the authoritative public catalog (currently only that runtime's `default` scope); unpublished named scopes are rejected before local history is opened. It does not start or alter runtime authentication. |
...
| API | `POST /api/auth-challenges/:id/cancel` | Available for an existing local challenge only | Browser-origin request plus bearer token required; body-bearing requests are rejected before state changes. The response is `account-center.auth-challenge-cancel.v1` and contains only redacted lifecycle metadata plus an optional expiry timestamp. This cancels Account Center's local challenge record; it does not alter runtime credentials. |
