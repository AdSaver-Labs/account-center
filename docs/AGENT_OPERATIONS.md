...
- All API responses use `Cache-Control: no-store` and contain redacted operational metadata only. In particular, `GET /api/status` projects reauthentication entries to lifecycle metadata and strips OAuth device codes and verification URLs even if an adapter's internal status object carries them.
...
The action catalog is authoritative for the running server. Its unavailable capability state is the literal uppercase `UNPROVEN` (distinct from any future lower-case result-union `kind`). An `available` action supplies its fixed local `endpoint` method and path; `:id` is an opaque path parameter, never an account identity or credential. No endpoint is supplied for blocked or `UNPROVEN` actions, so agents must not infer or construct a mutation URL. Agents must not assume that a planned UI/API feature exists merely because it appears in a roadmap.

The catalog advertises each protected local API action independently. Available endpoint bindings are method-enforced: a request using a different method receives `405 method_not_allowed` and an `Allow` header naming the single permitted method. Runtime mutations that remain unavailable declare `explicit_runtime_scope` as a prerequisite, so clients must not infer a default, named, or `all` scope while the protected contract and authoritative runtime proof are missing. Currently available local actions are `capabilities.list`, `status`, `models.list`, `runtime_scopes.list`, `auth_challenges.list`, `auth_challenges.detail`, `auth_challenges.cancel`, `audit.history`, and `mutation_operations.history`. `capabilities.list` is the protected self-discovery binding; clients may use its fixed `GET /api/capabilities` path before acting. `auth_challenges.cancel` changes only Account Center's local redacted challenge record and additionally requires a same-origin request; it accepts no request body and is not runtime authentication mutation.

## Current safe operations

| Surface | Operation | State | Agent rule |
|---|---|---|---|
| API | `GET /api/capabilities` | Available | Bearer token required; this is the catalog's protected self-discovery action and must be fetched before every workflow. |
...
| API | `GET /api/scopes` | Available for a read-only runtime scope catalog | Bearer token required; response is `account-center.runtime-scopes.v1` and contains only observed `openclaw`, `hermes`, or `codex` keys, their supported default scope, and declared capabilities. `generic-command` is a read-only source label, never a selectable public runtime or durable preference scope. Named agent/profile/session and `all` scopes are unavailable until authoritative runtime evidence exists. |
| API | `GET /api/auth-challenges?runtime=<observed>&scope=<observed>` | Available for local challenge inventory in its selected context only | Bearer token required; supply exactly one `runtime` and one `scope` from the authoritative public catalog (currently only that runtime's `default` scope). Missing, partial, duplicate, malformed, unpublished, or foreign selectors are rejected before local history is opened. The `account-center.auth-challenges.v1` response contains only redacted lifecycle metadata plus an optional expiry timestamp. It does not start or alter runtime authentication. |
| API | `GET /api/auth-challenges/:id?runtime=<observed>&scope=<observed>` | Available for an existing local challenge in its selected context only | Bearer token required. Supply exactly one `runtime` and one `scope` from the observed public scope catalog; missing, duplicate, malformed, or unpublished selectors fail closed before local lifecycle state is opened. The response is `account-center.auth-challenge.v1` and contains only redacted lifecycle metadata plus an optional expiry timestamp. An opaque ID is not sufficient by itself: an absent ID and an ID from a different selected context both return the same redacted `404`. This read does not start or alter runtime authentication. |
| API | `POST /api/auth-challenges/:id/cancel?runtime=<observed>&scope=<observed>` | Available for an existing local challenge in its selected context only | Browser-origin request plus bearer token required. Supply exactly one observed `runtime` and `scope`; missing, duplicate, malformed, or unpublished selectors fail closed before lifecycle-store access. Body-bearing requests are rejected before state changes. The response is `account-center.auth-challenge-cancel.v1` and contains only redacted lifecycle metadata plus an optional expiry timestamp. This cancels Account Center's local challenge record; it does not alter runtime credentials. |
...
| Credential delete | `account.delete` | Available only through the owned exact-account transaction | Preview by default; explicit `--apply` requires an exact connected target and invokes only the runtime-local `codex-auth-delete.py` transaction shared with Dexter `/auth delete`. The owner-controlled transaction provides backup, atomic rollback, and authoritative verification. Its sole public completion evidence is the target-free opaque receipt `opaque-owned-delete`; absent, malformed, or unverified native evidence is `UNPROVEN`. Do not use private bundled internals or direct SQLite/JSON edits. |
| Routing/model or guided-auth start/runtime mutation | Planned / `UNPROVEN` | Not available through the API yet; capability reasons are respectively `protected_route_contract_missing_scoped_review_idempotency_runtime_proof`, `protected_model_contract_missing_scoped_review_idempotency_runtime_proof`, and `protected_start_contract_missing_review_idempotency_runtime_proof`; each also requires an `explicit_runtime_scope` | Do not invoke private scripts as a substitute or infer a default/named/`all` scope. Durable challenge lifecycle metadata and cancellation are redacted local control-plane operations; neither proves a runtime mutation is available. |
