...
| `/auth delete <target>` | credential mutation | preview by default; only explicit `--apply` may invoke the owned native transaction | implied `--dry-run` | exact connected credential target, owner-only runtime-local backup, atomic rollback, one opaque redacted receipt, fresh authoritative proof |
...
- **delete** means credential deletion. `/auth delete <target>` is always a preview unless explicitly given `--apply`. Apply requires an exact connected-target match and, before any store change, the owned runtime-local transaction with backup, native atomic rollback, and fresh authoritative proof. Its only public completion proof is the target-free opaque receipt `opaque-owned-delete`; native stderr, target digest, paths, and store data never cross the adapter boundary. Direct JSON/SQLite edits and private runtime internals are not supported; Account Center uses neither direct store mutation nor an alternate delete helper. Absent verified evidence from the owned transaction, the result is blocked/`UNPROVEN`, never success.
