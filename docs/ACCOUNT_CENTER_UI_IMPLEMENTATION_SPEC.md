...
Current implementation note: `packages/cli/src/server.ts` provides bearer-protected, no-store `GET /api/status`; `GET /api/limits` and `GET /api/models` accept an optional runtime filter or an exact observed `runtime + scope=default` context and return only redacted inventory/catalog evidence; `GET /api/scopes` (observed runtime default scopes and declared capabilities only); redacted local challenge inventory/detail/cancellation routes; `GET /api/audit` (bounded redacted audit history, filterable by outcome/action/date and exact redacted runtime + scope kind when context is recorded); and bounded redacted protected-operation history plus opaque operation detail at `GET /api/mutation-operations` and `GET /api/mutation-operations/:operationId?runtime=:runtime&scopeKind=default`. Audit detail likewise requires `GET /api/audit/:auditId?runtime=:runtime&scopeKind=default`; an opaque ID alone is not an authority. It does **not** yet provide planned runtime mutation endpoints or authoritative named agent/profile/session/`all` scope catalogs. The implementation pass should preserve the existing no-store and bearer behavior while replacing the status-only shell only after the API contract exists.
